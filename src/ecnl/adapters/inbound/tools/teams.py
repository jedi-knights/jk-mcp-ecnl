...
def register_team_tools(mcp: FastMCP, service: ECNLService) -> None:
    ...
    async def get_teams(flight_id: int) -> str:
        ...
        return await _safe_call(service.get_teams(flight_id), _fmt_teams)
    ...
    async def get_clubs(event_id: int) -> str:
        ...
        return await _safe_call(service.get_clubs(event_id), _fmt_clubs)

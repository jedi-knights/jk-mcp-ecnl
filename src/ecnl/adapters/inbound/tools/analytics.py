...
def register_analytics_tools(mcp: FastMCP, service: ECNLService) -> None:
    ...
    async def get_results(event_id: int, flight_id: int) -> str:
        ...
        return await _safe_call(service.get_results(event_id, flight_id), _fmt_results)
    ...
    async def get_rpi(event_id: int, flight_id: int, tie_weight: float = 1 / 3) -> str:
        ...
        return await _safe_call(service.get_rpi(event_id, flight_id, tie_weight), _fmt_rpi)
    ...
    async def get_team_rpi(event_id: int, flight_id: int, team: str, tie_weight: float = 1 / 3) -> str:
        ...
        return await _safe_call(service.get_team_rpi(event_id, flight_id, team, tie_weight), _fmt_team_rpi)

...
def register_match_tools(mcp: FastMCP, service: ECNLService, authorizer: Authorizer) -> None:
    ...
    async def get_match(match_token: str) -> str:
        ...
        return await _safe_call_authorized(authorizer, "get_match", service.get_match(match_token), _fmt_raw)
    ...
    async def get_brackets(event_id: int, flight_id: int) -> str:
        ...
        return await _safe_call_authorized(
            authorizer, "get_brackets", service.get_brackets(event_id, flight_id), _fmt_raw
        )

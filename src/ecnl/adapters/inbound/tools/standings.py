...
def register_standings_tools(mcp: FastMCP, service: ECNLService, authorizer: Authorizer) -> None:
    ...
    async def get_standings(event_id: int, division_id: int, flight_id: int) -> str:
        ...
        return await _safe_call_authorized(
            authorizer, "get_standings", service.get_standings(event_id, division_id, flight_id), _fmt_standings
        )
